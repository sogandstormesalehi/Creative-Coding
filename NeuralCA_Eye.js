...
class NeuralCA {
    ...
    constructor() {
      this.W = new Float32Array([
        ...
        4,13,18,-57,-79,-22,-25,71,-12,-11,14,27,-7,-8,-7,6, 11,10,2,0,4,1,2,7,-22,-33,-15,-3,22,-107,14,-34]);
    ...
    frame(glsl, {DPR}) {
        ...
        void fragment() {
          ...
          if (s == vec4(0)) {
            ...
            FOut = 0.8+vec4(hash(I.xyy).x)*0.3;
            return;
          }
          vec2 dp = Src_step();
          float x = UV.x, y = UV.y;
          ...
          vec4 ds = rule(s-0.6, p);  // NCA rule application

          // Radial distortion effect
          vec2 center = vec2(0.5, 0.5); // Center of the image
          float dist = length(UV - center); // Distance from the center
          float distortion = 0.01 * sin(10.0 * dist * 2.5) * (1. - dist); // Sinusoidal distortion

          // Apply distortion to color channels
          vec4 distortedColor = s + ds * 1.5;
          distortedColor.rgb += distortion; // Modify the RGB channels based on distortion

          // Ensure the color values are in [0,1] range
          distortedColor = clamp(distortedColor, 0.0, 1.0);

          FOut = distortedColor;
        }`}, {story:2, scale:1/5/DPR, tag:'state'});
